export const levels = [
  ...
  {
    name: 'order 1',
    instructions: '<p>If grid items aren\'t explicitly placed with <code class="help">grid-area</code>, <code class="help">grid-column</code>, <code class="help">grid-row</code>, etc., they are automatically placed according to their order in the source code. We can override this using the <code class="help">order</code> property, which is one of the advantages of grid over table-based layout.</p><p>By default, all grid items have an <code class="help">order</code> of 0, but this can be set to any positive or negative value, similar to <code>z-index</code>.</p><p>Right now, the Human hero should be placed in the second column under the <strong>Orange Halo</strong> and Plati heroes in the rest of the places from 1st line, under the <strong>Blue Halo</strong>. Change the <code class="help">order</code> value of the halo!</p>',
    board: 'cwccc',
